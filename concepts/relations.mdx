---
title: "Relations: typed edges between knowledge nodes"
sidebarTitle: "Relations"
description: "Relations connect nodes with semantic meaning. BaBel+ supports nine directed relation types with configurable weights that control graph layout and AI suggestions."
---
...
<AccordionGroup>
  ...
  <Accordion title="similar_a — similar to">
    Links two thematically or stylistically similar pieces of content. Unlike all other types, `similar_a` is **only valid between nodes of the same type** (libro–libro, pelicula–pelicula, etc.).
...
<Tabs>
  ...
  <Tab title="similar_a constraint">
    When `type` is `similar_a`, both the source and target node must share the same `type` field (e.g. both `libro`, both `pelicula`). Creating a `similar_a` relation between nodes of different types returns `400 Bad Request`.
  </Tab>
</Tabs>
