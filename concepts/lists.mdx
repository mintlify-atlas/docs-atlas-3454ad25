---
title: "Lists: curated, ordered collections of nodes"
sidebarTitle: "Lists"
description: "Lists let you group nodes into named, ordered collections with per-entry ratings. Use lists to scope the graph view and organize reading queues or watch lists."
---
...
The `GET /api/lists/:id` endpoint returns a richer shape that includes the full node data for every member of the list, augmented with two list-specific fields:

```typescript
export interface ListWithNodes extends List {
  nodes: (Node & {
    position:   number        // 0-based integer; determines display order
    listRating: number | null // per-list rating, independent of the node's own rating
  })[]
}
```
