...
description: "Configure BaBel+ with environment variables covering the database connection, Groq API key, CORS, port, and JWT secret. Required vs optional explained."
---
...
The root `.env.example` (monorepo root) documents the client-side variable `VITE_API_URL` alongside `GROQ_API_KEY` and `JWT_SECRET`. See [VITE_API_URL](#vite_api_url) below for details.
